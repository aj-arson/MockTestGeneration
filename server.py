...
if not _API_KEY:
    print("WARNING: API_KEY not set in environment. All endpoints are unprotected.")
...
class BackGroundWorker(threading.Thread):
    ...
    def process(self, req:MockTest):
        ...
        try:
            ...
            print(f"Exception in process(server.py): {traceback.format_exc()}")
            try:
                if db and cursor:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print(f"Exception in run(server.py): {traceback.format_exc()}")
                time.sleep(60)
            finally:
                try:
                    cursor.close()
...
async def queue_mock_test(req:MockTest):
    ...
    try:
        ...
        print(f"Exception in queue_test(server.py): {traceback.format_exc()}")
...
def get_mock_test(TestID):
    ...
    try:
        ...
        print(f"Exception in get_mock_test(server.py): {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail="Internal server error.")
    finally:
        if cursor: cursor.close()
        if db: db.close()
...
def clear_table_records():
    ...
    try:
        ...
        print(f"Exception in clear_table_records(server.py): {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail="Internal server error.")
    finally:
        if cursor: cursor.close()
        if db: db.close()
...
async def queue_engineering_stream_test(req: MockTest):
    ...
    try:
        ...
        print(f"Exception in queue_engineering_stream_test(server.py): {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail="Internal server error.")
    finally:
        if cursor: cursor.close()
        if db: db.close()
...
async def queue_agriculture_pharmacy_stream_test(req: MockTest):
    ...
    try:
        ...
        print(f"Exception in queue_agriculture_pharmacy_stream_test(server.py): {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail="Internal server error.")
    finally:
        if cursor: cursor.close()
        if db: db.close()
...
class EAMCET160BackgroundWorker(threading.Thread):
    ...
    def process(self, req: MockTest):
        ...
        try:
            ...
            print(f"Exception in EAMCET160 process(server.py): {traceback.format_exc()}")
            try:
                if db and cursor:
                    save_generated_questions_to_db(db=db, cursor=cursor, TestID=req.TestID, questions=None)
                    set_generation_status(db=db, cursor=cursor, TestID=req.TestID, Generation_status=Status.FAILED.value)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print(f"Exception in EAMCET160 run(server.py): {traceback.format_exc()}")
                time.sleep(60)
            finally:
                try:
                    cursor.close()
...
async def queue_eamcet160_engineering(req: EAMCET160Test):
    ...
    if req.Stream != 'Engineering':
        raise HTTPException(status_code=400, detail=f"Invalid Stream '{req.Stream}'. Expected 'Engineering'.")
    db, cursor = None, None
    try:
        ...
        print(f"Exception in queue_eamcet160_engineering(server.py): {traceback.format_exc()}")
...
async def queue_eamcet160_agriculture(req: EAMCET160Test):
    ...
    if req.Stream != 'Agriculture_and_Pharmacy':
        raise HTTPException(status_code=400, detail=f"Invalid Stream '{req.Stream}'. Expected 'Agriculture_and_Pharmacy'.")
    db, cursor = None, None
    try:
        ...
        print(f"Exception in queue_eamcet160_agriculture(server.py): {traceback.format_exc()}")
